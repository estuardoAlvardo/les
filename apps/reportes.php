         ...
         <div class=" col-xs-12" style="height:50px; margin-bottom: 50px;">
             <form method="get" action="reportes.php" class="form-inline form-filtro " id="formulario">
                  ...
                  <div class="form-group" style="">
                    ...
                    <select class="form-control" name="preescolarSet"  style="margin-left: 20px; <?php echo $preco1; ?>" >
...
if(@(int)$_GET['preescolarSet']!=0 and @(int)$_GET['semana']>=1){
  ...
  $semana=@(int)$_GET['semana'];
   $nivel='precolar';
  buscarNiveles($gradoReporte,$semana,$nivel);
...
function buscarNiveles($gradoReporte,$semana,$nivel){
 ...
 <div class="col-md-2 sombra text-left" style="height:25px; margin-top:50px; margin-bottom: -170px; background-color: #2980b9; color: white; border-radius: 5px;">Lecturas Diarias</div>
          ...
          <div class="col-md-12 sombra"  id="customers" style=" min-height:100px; margin-bottom: 30px;">
                    ...
                    <table class="table table-hover" id="lectDiaria">
                       <caption style="display: none;"><?php echo 'Grado: '.$gradoReporte.'-- Semana '.$semana.'<br> Reporte a fecha: '.$fecha_actual.' '.$hora_actual; ; ?></caption>
                      <thead>
                       ...
                       <td><strong><?php echo $rowr2['nombre'].' '.$rowr2['apellido']; ?></strong></td>  
                        <td><strong><?php echo $semana; ?></strong></td>   
                          ...
                          $buscarInCnb= ("SELECT * FROM registropruebacomprension JOIN cuestionario on registropruebacomprension.idLectura=cuestionario.idLectura where registropruebacomprension.idUsuario=:idUsuario and registropruebacomprension.intento=1 and cuestionario.fundamento='cnb' and registropruebacomprension.idLectura=:idLectura");
                            ...
                            if($gradoReporte==3){
                               $buscarInCnb= ("SELECT * FROM registropruebacomprension3p JOIN cuestionario on registropruebacomprension3p.idLectura=cuestionario.idLectura where registropruebacomprension3p.idUsuario=:idUsuario and registropruebacomprension3p.intento=1 and cuestionario.fundamento='cnb' and registropruebacomprension3p.idLectura=:idLectura");
                               $buscarCnb=$dbConn->prepare($buscarInCnb);
                                  $buscarCnb->bindParam(':idLectura',$_SESSION['idLectura'], PDO::PARAM_INT);
                                  $buscarCnb->bindParam(':idUsuario',$rowr2['idUsuario'], PDO::PARAM_INT);
                                   $buscarCnb->execute(); 
                            ...
                            if($gradoReporte==4){
                             ...
                             if($gradoReporte==5){
                               $buscarInCnb= ("SELECT * FROM registropruebacomprension5p JOIN cuestionario on registropruebacomprension5p.idLectura=cuestionario.idLectura where registropruebacomprension5p.idUsuario=:idUsuario and registropruebacomprension5p.intento=1 and cuestionario.fundamento='cnb' and registropruebacomprension5p.idLectura=:idLectura");
                               $buscarCnb=$dbConn->prepare($buscarInCnb);
                                  $buscarCnb->bindParam(':idLectura',$_SESSION['idLectura'], PDO::PARAM_INT);
                                  $buscarCnb->bindParam(':idUsuario',$rowr2['idUsuario'], PDO::PARAM_INT);
                                   $buscarCnb->execute(); 
                            ...
                            if($gradoReporte>=6 and $gradoReporte<=11){
                               $buscarInCnb= ("SELECT * FROM registropruebacomprension6p JOIN cuestionario on registropruebacomprension6p.idLectura=cuestionario.idLectura where registropruebacomprension6p.idUsuario=:idUsuario and registropruebacomprension6p.intento=1 and cuestionario.fundamento='cnb' and registropruebacomprension6p.idLectura=:idLectura");
                               $buscarCnb=$dbConn->prepare($buscarInCnb);
                                  $buscarCnb->bindParam(':idLectura',$_SESSION['idLectura'], PDO::PARAM_INT);
                                  $buscarCnb->bindParam(':idUsuario',$rowr2['idUsuario'], PDO::PARAM_INT);
                                   $buscarCnb->execute(); 
                          ...
                          <td>
                           ...
                           $buscarCantidadPalabras= ("SELECT * FROM palabrasglosario as palabras JOIN glosario as glo on palabras.idGlosario=glo.idglosario WHERE glo.idLectura=:idLectura");
                           ...
                           //consulta para ver cuantas palabras lleva
                               $buscarRegistroP= ("SELECT * from registroglosario as re JOIN glosario as glo on re.idGlosario=glo.idglosario where   re.idUsuario=:idUsuario and glo.idLectura=:idLectura");
...
<?php $gradoReporte=0; $semana=0; } ?>
 ...
 <div class="dropdown botonAgg botonAgg-1" id="quitarPorBusqueda" style="margin-top: 20px; background-color: #3498db; padding: 20px; color: white; <?php echo $hayBusqueda1; ?>" ><h1>No se ha realizado ninguna busqueda.</h1></div>
